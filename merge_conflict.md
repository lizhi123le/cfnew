# 合并冲突报告
## 冲突时间: Tue Jun  9 11:50:00 UTC 2026
## 上游更新哈希: 5b20c42652e72d2f363c2ae59f2a0cf760a14856862c43796299d2281b6664ab
...
<<<<<<< local_明文源吗
    ...
    let customECHDomain = '';
    // ECH 配置缓存（内存 + KV 冷触发混合模式）
    const echConfigCache = new Map();
    const echConfigCacheTime = new Map();
    const ECH_CACHE_TTL = 21600000; // 6 小时
    // 自定义ALPN（可选：h3, h2, http/1.1，逗号分隔）
    let customALPN = '';
    // Cache API 缓存
    ...
    let kvStore = null;
    let kvConfig = {};
    let kvConfigCache = null;       // 内存缓存（深拷贝隔离，防污染）
    let kvConfigCacheTime = 0;      // 缓存时间戳
    const KV_CACHE_TTL = 259200000; // 3 天缓存（冷触发混合模式）
    let kvConfigVersion = ''; 
    ...
    // ALPN 规范化：只允许 h3, h2, http/1.1 及其组合
    ...
    async function loadKVConfig(force = false) {
        ...
        // 冷触发混合模式：TTL 内从缓存深拷贝，零 KV 操作
        if (!force && kvConfigCache && Date.now() - kvConfigCacheTime < KV_CACHE_TTL) {
            kvConfig = JSON.parse(JSON.stringify(kvConfigCache));
            return;
        }

        try {
            // 版本检查：未变化则从缓存深拷贝（仅刷新时间戳）
            let ver = '';
            try { ver = (await kvStore.get('c_ver')) || ''; } catch (_) {}
            if (!force && ver && ver === kvConfigVersion && kvConfigCache) {
                kvConfigCacheTime = Date.now();
                kvConfig = JSON.parse(JSON.stringify(kvConfigCache));
                return;
            }

            // 随机延迟 0-300ms，缓解 KV 并发
            if (!force) {
            ...
            // KV 读取失败时保留现有缓存，避免临时故障导致配置丢失
            if (!kvConfig) kvConfig = {};
            if (!kvConfigCache) kvConfigCache = {};
        } catch (error) {
            if (!kvConfig) kvConfig = {};
            if (!kvConfigCache) kvConfigCache = {};
        }
    ...
    export default {
        async fetch(request, env, ctx) {
            try {
                ...
                adminPassword = env.ADMIN || env.admin || '';
                ...
                // 从 host 配置填充 hostList（供 getRandomHost 使用）
                if (host && host.trim()) {
                ...
                const randomPathControl = getConfigValue('randomPath', env.randomPath || env.RANDOMPATH);
                if (randomPathControl !== undefined && randomPathControl !== '') {
                    enableRandomPath = randomPathControl === 'yes' || randomPathControl === true || randomPathControl === 'true' || randomPathControl === '1';
                }

                const url = new URL(request.url);

                // 根路径管理员登录 API（主域名终端面板认证用）
                ...
                if (url.pathname.includes('/api/config')) {
                    ...
                    if (cp && cp.trim()) {
                        ...
                        if (isValid) {
                            // 需要管理员认证
                            if (adminPassword && !(await isAdminAuthenticated(request))) {
                                return new Response(JSON.stringify({ error: '未登录', message: '需要管理员登录' }), {
                                    status: 401,
...
<<<<<<< local_明文源吗
                ...
                if (request.method === 'GET') {
                    ...
                    if (url.pathname === '/') {
                        ...
                        // 检查管理员认证状态（用于终端显示密码提示）
                        const requiresAdminAuth = adminPassword && !(await isAdminAuthenticated(request));
                        
                        // 管理员已认证 → 直接跳转到订阅配置页面，不显示终端
                        if (adminPassword && !requiresAdminAuth) {
                            const targetPath = cp && cp.trim() ? '/' + cp.trim().replace(/^\//, '') : '/' + at;
                            return Response.redirect(new URL(targetPath, request.url).toString(), 302);
                        }
    ...
    <body>
        ...
        <div class="terminal">
            ...
            <div class="terminal-body" id="terminalBody">
                <!-- 管理员登录提示（当 env.ADMIN 已设置且未认证时显示） -->
                <div id="adminLoginSection" style="display:${requiresAdminAuth ? 'block' : 'none'}">
...
<<<<<<< local_明文源吗
            ...
            // 管理员登录成功后跳转的订阅配置页面地址（拼接完整 URL，避免路径解析异常）
            var adminRedirectUrl = window.location.origin + '${cp && cp.trim() ? '/' + cp.trim().replace(/^\//, '') : '/' + at}';

            document.addEventListener('DOMContentLoaded', function() {
                try { createMatrixRain(); } catch (e) {}
                const requiresAuth = ${requiresAdminAuth ? 'true' : 'false'};
                if (requiresAuth) {
                    const adminInput = document.getElementById('adminPwdInput');
                    if (adminInput) {
                        adminInput.focus();
    ...
    function parseLinkToClashNode(link) {
        try {
            ...
            if (link.startsWith('vless://')) {
                ...
                if (ech) {
                    const echDomain = (ech.includes('+') ? ech.split('+')[0] : ech) || getRandomHost(workerDomain) || workerDomain;
                    node['ech-opts'] = {
                        enable: true,
...
<<<<<<< local_明文源吗
    ...
    function buildClashNodeLine(n) {
        ...
        if (n.ech) {
            const echDomain = (n.ech.includes('+') ? n.ech.split('+')[0] : n.ech) || getRandomHost(workerDomain) || workerDomain;
            lines.push(`    ech-opts:`);
    ...
    async function generateSingBoxJson(links) {
        const nodes = links.map(parseShareLink).filter(n => n && (n.proto === 'vless' || n.proto === 'trojan'));
        const dnsServer = customDNS || 'https://223.5.5.5/dns-query';
        const outboundTags = nodes.map(n => n.name);

        async function nodeToOutbound(n) {
            const out = {
                type: n.proto,
                tag: n.name,
        ...
        const config = {
            ...
            outbounds: [
                ...
                ...await Promise.all(nodes.map(nodeToOutbound)),
...
<<<<<<< local_明文源吗
    ...
    async function generateSingBoxConfig(links) { return generateSingBoxJson(links); }
    ...
    // 通过 DoH（DNS-over-HTTPS）查询指定域名的 DNS 记录，返回解析后的答案数组
    ...
    async function fetchECHConfig(domain) {
        ...
        // L1: 内存缓存（同域名不读KV，仅内存）
        const cached = echConfigCache.get(domain);
        const cachedTime = echConfigCacheTime.get(domain);
        if (cached && cachedTime && Date.now() - cachedTime < ECH_CACHE_TTL) {
            echDebugInfo = `✅ 使用缓存中的 ${domain} ECH 配置`;
            return cached;
        }

        // L2: KV 冷触发缓存（仅内存未命中时读取，恢复后同域名不再读KV）
        if (kvStore) {
            try {
                const kvData = await kvStore.get('ech_' + domain);
                if (kvData) {
                    const parsed = JSON.parse(kvData);
                    if (parsed && parsed.config && parsed.time && Date.now() - parsed.time < ECH_CACHE_TTL) {
                        echConfigCache.set(domain, parsed.config);
                        echConfigCacheTime.set(domain, parsed.time);
                        echDebugInfo = `✅ 从 KV 缓存恢复 ${domain} 的 ECH 配置`;
                        return parsed.config;
                    }
                }
            } catch (_) { /* KV 读取失败，降级到 DoH */ }
        }

        // L3: DoH 查询
        try {
            const dnsServer = customDNS || 'https://cloudflare-dns.com/dns-query';
            const answers = await DoH查询(domain, 'HTTPS', dnsServer);
            if (!answers.length) {
                echDebugInfo = `❌ ${domain} 的 HTTPS/SVCB 记录未找到`;
                return null;
            }
            for (const ans of answers) {
                ...
                while (offset + 4 <= bytes.length) {
                    ...
                    offset += 4;
                    if (key === 5) {
                        const config = btoa(String.fromCharCode(...bytes.slice(offset, offset + len)));
                        const now = Date.now();
                        // 回填 L1 内存缓存
                        echConfigCache.set(domain, config);
                        echConfigCacheTime.set(domain, now);
                        // 回填 L2 KV 缓存（全局持久化，供冷启动恢复）
                        if (kvStore) {
                            try {
                                await kvStore.put('ech_' + domain, JSON.stringify({ config, time: now }));
                            } catch (_) { /* KV 写入失败不影响主流程 */ }
                        }
                        echDebugInfo = `✅ 成功通过 DoH 获取 ${domain} 的 ECH 配置`;
                        return config;
                    }
                    offset += len;
                }
            }
            echDebugInfo = `❌ ${domain} 的 HTTPS/SVCB 记录中未找到 ECH 配置`;
            return null;
        } catch (error) {
            echDebugInfo = `❌ 获取 ECH 配置失败: ${error.message}`;
            return null;
=======
...
<<<<<<< local_明文源吗
    ...
    async function handleSubscriptionRequest(request, user, url = null) {
        ...
        // Cache API: 尝试获取缓存
        const cachedResponse = await cfCache.match(request);
        ...
        if (enableECH) {
            const dnsServer = customDNS || 'https://223.5.5.5/dns-query';
            const echDomain = customECHDomain || getRandomHost(workerDomain) || workerDomain;
            echConfig = `${echDomain}+${dnsServer}`;
        }

        // 全局过滤：IP 版本、域名类型、运营商（提取为函数级，同时给 addNodesFromList 和 yxURL 使用）
        const filterDomainEnabled = getConfigValue('domain', '') === '' || getConfigValue('domain', 'yes') !== 'no';
        const filterIpv4Enabled = getConfigValue('ipv4', '') === '' || getConfigValue('ipv4', 'yes') !== 'no';
        const filterIpv6Enabled = getConfigValue('ipv6', '') === '' || getConfigValue('ipv6', 'yes') !== 'no';
        const filterIspMobile = getConfigValue('ispMobile', '') === '' || getConfigValue('ispMobile', 'yes') !== 'no';
        const filterIspUnicom = getConfigValue('ispUnicom', '') === '' || getConfigValue('ispUnicom', 'yes') !== 'no';
        const filterIspTelecom = getConfigValue('ispTelecom', '') === '' || getConfigValue('ispTelecom', 'yes') !== 'no';
        const isNodeAllowed = (item) => {
            const host = item.ip || '';
            ...
            if (isIPv6 && !filterIpv6Enabled) return false;
            if (isIPv4 && !filterIpv4Enabled) return false;
            if (isDomain && !filterDomainEnabled) return false;
            const isp = (item.isp || item.name || '').toLowerCase();
            if (isp.includes('移动') || isp.includes('mobile')) { if (!filterIspMobile) return false; }
            if (isp.includes('联通') || isp.includes('unicom')) { if (!filterIspUnicom) return false; }
            if (isp.includes('电信') || isp.includes('telecom')) { if (!filterIspTelecom) return false; }
            return true;
        };

        async function addNodesFromList(list) {
            const filtered = list.filter(isNodeAllowed);
            if (filtered.length > 0) allSources.push(filtered);
        }
        ...
        // 4. yxURL / GitHub 源（独立于 hasCustomPreferred，与旧版共存逻辑一致）
        if (!disablePreferred && egi) {
            try {
                const newIPList = await fetchAndParseNewIPs();
                if (newIPList.length > 0) {
                    // 应用全局过滤（IP版本、运营商）
                    const filteredNewIPs = newIPList.filter(item => isNodeAllowed({ ip: item.ip, name: item.name }));
                    if (filteredNewIPs.length > 0) {
                        allNewIPs.push(filteredNewIPs);
                    }
                }
            } catch (error) {
                if (!currentWorkerRegion) {
        ...
        const generateFromSources = async (type) => {
            for (const src of allSources) {
                if (type === 'vless' && ev) finalLinks.push(...await generateLinksFromSource(src, user, workerDomain, echConfig));
                if (type === 'trojan' && et) finalLinks.push(...await generateTrojanLinksFromSource(src, user, workerDomain, echConfig));
                if (type === 'xhttp' && ex) finalLinks.push(...await generateXhttpLinksFromSource(src, user, workerDomain, echConfig));
            }
        };
        const generateFromNewIPs = async (type) => {
            for (const ipList of allNewIPs) {
                if (type === 'vless' && ev) finalLinks.push(...await generateLinksFromNewIPs(ipList, user, workerDomain, echConfig));
                if (type === 'trojan' && et) finalLinks.push(...await generateTrojanLinksFromNewIPs(ipList, user, workerDomain, echConfig));
                if (type === 'xhttp' && ex) finalLinks.push(...await generateXhttpLinksFromSource(ipList, user, workerDomain, echConfig));
            }
        };
        // A. VLESS 节点 (最前面)
        await generateFromSources('vless');
        await generateFromNewIPs('vless');
        // B. Trojan 节点 (中间)
        await generateFromSources('trojan');
        await generateFromNewIPs('trojan');
        // C. xhttp 节点 (最后)
        await generateFromSources('xhttp');
        await generateFromNewIPs('xhttp');
...
<<<<<<< local_明文源吗
        ...
        switch (target.toLowerCase()) {
            ...
            case 'sing-box':
            case 'singbox':
                subscriptionContent = await generateSingBoxJson(finalLinks);
        ...
        const responseHeaders = { 
            'Content-Type': contentType,
            'Cache-Control': 'public, max-age=720, s-maxage=300',
        };
        ...
        const response = new Response(subscriptionContent, {
            headers: responseHeaders,
        });
        // Cache API: 存储响应（异步，静默失败）
        cfCache.put(request, response.clone()).catch(() => {});
        return response;
=======
...
<<<<<<< local_明文源吗
    async function generateLinksFromSource(list, user, workerDomain, echConfig = null, skipNumbering = false) {
        ...
        for (const item of list) {
            if (++yieldCounter % 50 === 0) await new Promise(r => setTimeout(r, 0));
            let nodeNameBase = (item.isp || '').split(/[?#@]/)[0].trim().replace(/\s/g, '_') || item.ip.replace(/\s/g, '_');
            if (item.colo && item.colo.trim()) {
                nodeNameBase = `${nodeNameBase}-${item.colo.trim()}`;
            }
            const safeIP = item.ip.includes(':') && !item.ip.includes('[') ? `[${item.ip}]` : item.ip;
            const nodeHost = getRandomHost(workerDomain) || workerDomain;
            const wsPath = enableRandomPath ? randomPath('/?ed=2560') : '/?ed=2560';
            ...
            for (const { port, tls } of portsToGenerate) {
                ...
                if (tls) {
                    ...
                    if (enableECH) {
                        ...
                        const echConfig = await fetchECHConfig(echDomain);
                        if (echConfig) wsParams.set('echconfiglist', echConfig);
                    }
...
<<<<<<< local_明文源吗
    ...
    // 创建登录 token：HMAC-SHA256 签名 + UA 绑定，不包含密码明文
    async function createAdminToken(password, request) {
        const t = Date.now();
        const encoder = new TextEncoder();

        // 计算 User-Agent SHA-256 哈希用于绑定
        const ua = request.headers.get('User-Agent') || '';
        const uaHash = await crypto.subtle.digest('SHA-256', encoder.encode(ua));
        const uaArr = new Uint8Array(uaHash);
        let uaBin = '';
        for (let i = 0; i < uaArr.length; i++) uaBin += String.fromCharCode(uaArr[i]);
        const uaB64 = btoa(uaBin);

        const payload = JSON.stringify({ t, ua: uaB64 });
        ...
        return payloadB64 + '.' + btoa(sigBin);
    }

    // 验证登录 token：HMAC 验签 + UA 绑定校验 + 1 小时内
    async function validateAdminToken(token, password, request) {
        try {
            const parts = token.split('.');
            if (parts.length !== 2) return false;
            const [payloadB64, sigB64] = parts;
            const data = JSON.parse(atob(payloadB64));
            if (Date.now() - data.t > 3600000) return false; // 1 小时

            const encoder = new TextEncoder();

            // 校验 User-Agent 绑定
            const currentUA = request.headers.get('User-Agent') || '';
            const uaHash = await crypto.subtle.digest('SHA-256', encoder.encode(currentUA));
            const uaArr = new Uint8Array(uaHash);
            let uaBin = '';
            for (let i = 0; i < uaArr.length; i++) uaBin += String.fromCharCode(uaArr[i]);
            const currentUAB64 = btoa(uaBin);
            if (data.ua !== currentUAB64) return false;
    ...
    async function isAdminAuthenticated(request) {
        if (!adminPassword) return true; // 未设置密码，无需认证
        const cookieHeader = request.headers.get('Cookie') || '';
        if (!cookieHeader) return false;
        const cookies = cookieHeader.split(';').map(c => c.trim());
        for (const cookie of cookies) {
            if (cookie.startsWith('cfnew_admin_token=')) {
                const token = cookie.substring('cfnew_admin_token='.length);
                return await validateAdminToken(token, adminPassword, request);
            }
        }
        return false;
    }
    ...
    async function handleSubscriptionPage(request, user = null) {
        ...
        const translations = {
            zh: {
                ...
                customPath: '自定义路径 (d):',
                customIP: '自定义ProxyIP (p):',
                hostSetting: '自定义HOST (host):',
                hostPlaceholder: '例如: h1.com,h2.com',
                preferredIPs: '优选IP列表 (yx):',
                preferredIPsURL: '优选IP来源URL (yxURL):',
                latencyTest: '延迟测试',
                latencyTestIP: '测试IP/域名:',
            ...
            fa: {
                ...
                customPath: 'مسیر سفارشی (d):',
                customIP: 'ProxyIP سفارشی (p):',
                hostSetting: 'HOST سفارشی (host):',
                hostPlaceholder: 'مثال: h1.com,h2.com',
                preferredIPs: 'لیست IP ترجیحی (yx):',
                preferredIPsURL: 'URL منبع IP ترجیحی (yxURL):',
                latencyTest: 'تست تاخیر',
                latencyTestIP: 'IP/دامنه تست:',
...
>>>>>>> upstream_明文源吗
    ...
    <body>
        ...
        <div class="matrix-code-rain" id="matrixCodeRain"></div>
            ...
            ${adminPassword ? `
            <button type="button" class="cp-logout-btn" onclick="handleLogout()" title="${isFarsi ? 'خروج' : '登出'}">${isFarsi ? 'خروج' : '登出'}</button>
            ` : ''}
            <div class="cp-lang-wrapper">
                <span class="cp-lang-tag">LANG_</span>
                <select id="languageSelector" onchange="切换语言(this.value)">
            ...
            <div class="card" id="configCard" style="display: none;">
                ...
                <div id="configContent" style="display: none;">
                    ...
                    <form id="otherConfigForm" style="margin-bottom: 20px;">
                                ...
                                <div style="margin-bottom: 10px;">
                                    <label style="display: inline-flex; align-items: center; cursor: pointer; color: #00f0ff;">
                                        <input type="checkbox" id="randomPath" style="margin-right: 8px; width: 18px; height: 18px; cursor: pointer;">
                                            <span style="font-size: 1.1rem;">${t.enableRandomPath}</span>
                                    </label>
                                    <small style="color: #7aa9c4; font-size: 0.8rem; display: block; margin-top: 5px; margin-left: 26px;">${t.enableRandomPathHint}</small>
                                </div>
                                <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(0, 240, 255, 0.3);">
                        ...
                        <div style="margin-bottom: 15px;">
<<<<<<< local_明文源吗
                                <label style="display: block; margin-bottom: 8px; color: #00f0ff; font-weight: bold; text-shadow: 0 0 3px #00f0ff;">${t.hostSetting}</label>
                                <input type="text" id="hostInput" placeholder="${t.hostPlaceholder}" style="width: 100%; padding: 12px; background: rgba(0, 0, 0, 0.8); border: 2px solid #00f0ff; color: #00f0ff; font-family: 'Courier New', monospace; font-size: 14px;">
                                <small style="color: #7aa9c4; font-size: 0.85rem;">${isFarsi ? 'HOST سفارشی گره‌ها (اختیاری). برای انتخاب تصادفی از کاما برای جدا کردن چندین استفاده کنید.' : '自定义节点的HOST值（可选）。支持多个用逗号分隔，系统自动随机选择。'}</small>
                        </div>
                        <div style="margin-bottom: 15px;">
...
<<<<<<< local_明文源吗
            ...
            async function loadCurrentConfig() {
                ...
                try {
                    ...
                    document.getElementById('randomPath').checked = config.randomPath === 'yes';
                ...
                if (document.getElementById('domainEnabled')) document.getElementById('domainEnabled').checked = config.domain !== 'no';
            ...
            // ECH 开启时锁定 ALPN，关闭时恢复（在 loadCurrentConfig 和 change 事件中共用）
            function updateECHLockState() {
                const cb = document.getElementById('ech');
                const alpn = document.getElementById('alpn');
                if (!cb || !alpn) return;
                if (cb.checked) {
                    const currentVal = alpn.value;
                    if (currentVal) localStorage.setItem('ech_alpn_backup', currentVal);
                    alpn.disabled = true;
                    alpn.style.opacity = '0.5';
                    alpn.value = '';
                } else {
                    const backup = localStorage.getItem('ech_alpn_backup');
                    alpn.value = backup || alpn.value;
                    alpn.disabled = false;
                    alpn.style.opacity = '1';
                }
            }
            ...
            async function saveConfig(configData) {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        // 更新wk地区选择状态
                        updateWkRegionState();
                        // 预取 /sub 以更新浏览器缓存中的 ECH 状态
                        try {
                            await fetch(window.location.href + '/sub', { cache: 'reload' });
                        } catch (_) {}
                        // 保存成功后刷新页面以更新系统状态
                        setTimeout(function() {
                            window.location.reload();
                ...
                if (confirm('确定要重置所有配置吗？这将清空所有KV配置，恢复为环境变量设置。')) {
                    try {
                        ...
                        host: '',
                                ...
                                qj: '',
                                dkby: '',
                                yxby: '', ev: '', et: '', ex: '', ech: '', randomPath: '', tp: '', epd: '', epi: '', egi: '',
                                ipv4: '', ipv6: '', domain: '', ispMobile: '', ispUnicom: '', ispTelecom: '',
                                homepage: '',
                                alpn: ''
                            })
                        });
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // ECH 开启时自动联动开启仅TLS，并锁定ALPN设置（ECH不支持ALPN）
                ...
                function collectAllConfig() {
                    ...
                    return {
                        wk: val('wkRegion'),
                        ev: chk('ev', 'yes'), et: chk('et', 'no'), ex: chk('ex', 'no'), ech: chk('ech', 'no'), randomPath: chk('randomPath', 'no'),
                        tp: val('tp'),
...
<<<<<<< local_明文源吗
            ...
            const apiResults = await fetchPreferredAPI(urls, '443', 8000);
            ...
            if (apiResults.length > 0) {
                ...
                for (const item of apiResults) {
                    ...
                    const isIPv6 = item.includes(':') && !item.includes('[') && (item.split(':').length > 2);
                    
                    if (match) {
                        // 对 name 做 safe 处理，去除 ? 后方查参
                        const rawName = match[3]?.trim() || '';
                        const safeName = /[?@]/.test(rawName) ? rawName.split(/[?#]/)[0].trim() || match[1] : (rawName || match[1]);
                        results.push({
                            ip: match[1],
                        ...
                        let hostPart = hashIndex > -1 ? item.substring(0, hashIndex) : item;
                        const remark = hashIndex > -1 ? item.substring(hashIndex + 1) : '';
                        ...
                        if (lastColonIndex > 0 && /^\d+$/.test(possiblePort) && !hostPart.includes('::' + possiblePort)) {
                            port = parseInt(possiblePort, 10);
                            ip = hostPart.substring(0, lastColonIndex);
                        }
                        
                        const rawName = remark.trim();
                        const safeName = /[?@]/.test(rawName) ? rawName.split(/[?#]/)[0].trim() || ip : (rawName || ip);
                        results.push({
                            ip: ip,
                            port: port,
                            name: safeName
            ...
            for (const line of lines) {
                ...
                const isIPv6 = trimmedLine.includes(':') && (trimmedLine.split(':').length > 2);
                ...
                if (match) {
                    ...
                    let hostPart = hashIndex > -1 ? trimmedLine.substring(0, hashIndex) : trimmedLine;
                    const remark = hashIndex > -1 ? trimmedLine.substring(hashIndex + 1) : '';
...
<<<<<<< local_明文源吗
    ...
    async function generateLinksFromNewIPs(list, user, workerDomain, echConfig = null, skipNumbering = false) {
        const CF_HTTP_PORTS = [80, 8080, 8880, 2052, 2082, 2086, 2095];
        const CF_HTTPS_PORTS = [443, 2053, 2083, 2087, 2096, 8443];
        const links = [];
        const proto = atob('dmxlc3M=');
        ...
        for (const item of list) {
            if (++yieldCounter % 50 === 0) await new Promise(r => setTimeout(r, 0));
            const nodeNameBase = (item.name || '').split(/[?#@]/)[0].trim().replace(/\s/g, '_') || item.ip.replace(/\s/g, '_');
            const port = item.port;
            const safeIP = item.ip.includes(':') && !item.ip.includes('[') ? `[${item.ip}]` : item.ip;
            const nodeHost = getRandomHost(workerDomain) || workerDomain;
            const wsPath = enableRandomPath ? randomPath('/?ed=2560') : '/?ed=2560';
            ...
            if (CF_HTTPS_PORTS.includes(port) || !CF_HTTP_PORTS.includes(port)) {
                // TLS 端口或未知端口（默认TLS）
                const suffix = '-WS-TLS';
                ...
                // 如果配置了ALPN，添加alpn参数（ECH启用时禁用ALPN）
                if (!enableECH && customALPN) wsParams.set('alpn', customALPN);
                // 如果启用了ECH，添加ech参数
                if (enableECH) {
                    const dnsServer = customDNS || 'https://223.5.5.5/dns-query';
                    const echDomain = customECHDomain || getRandomHost(workerDomain) || workerDomain;
...
<<<<<<< local_明文源吗
    ...
    async function generateXhttpLinksFromSource(list, user, workerDomain, echConfig = null, skipNumbering = false) {
        const links = [];
        ...
        for (const item of list) {
            if (++yieldCounter % 50 === 0) await new Promise(r => setTimeout(r, 0));
            let nodeNameBase = (item.isp || item.name || item.ip || '').split(/[?#@]/)[0].trim().replace(/\s/g, '_');
            ...
            const params = new URLSearchParams({
                ...
                type: 'xhttp',
                host: nodeHost,
                path: enableRandomPath ? randomPath('/' + nodePath) : '/' + nodePath,
                mode: 'stream-one'
            });
            ...
            if (enableECH) {
                ...
                const echConfig = await fetchECHConfig(echDomain);
                if (echConfig) params.set('echconfiglist', echConfig);
            }
    ...
    function randomPath(fullNodePath = "/") {
        ...
        const randomPath = commonPathDirs.slice().sort(() => 0.5 - Math.random()).slice(0, randomCount).join('/');
    ...
    function replaceWildcards(str) {
        return str
            ...
            .replace(/\$\{random\}/g, Math.random().toString(36).substring(2, 8))
            .replace(/\$\{uuid\}/g, 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
                const r = Math.random() * 16 | 0;
                return (c === 'x' ? r : (r & 0x3 | 0x8)).toString(16);
            }));
    }

    function getRandomHost(fallback) {
        if (hostList && hostList.length > 0) {
            const randomIndex = Math.floor(Math.random() * hostList.length);
            return replaceWildcards(hostList[randomIndex]);
        }
        return replaceWildcards(fallback);
    }
    ...
    async function handleConfigAPI(request) {
        if (request.method === 'GET') {
            ...
            try {
                ...
                // 清除订阅缓存，使 ECH 状态等立即更新
                try {
                    const reqUrl = new URL(request.url);
                    const basePath = reqUrl.pathname.replace(/\/api\/config.*$/, '');
                    const subUrl = reqUrl.origin + basePath + '/sub';
                    await cfCache.delete(new Request(subUrl));
                } catch (_) {}
...
<<<<<<< local_明文源吗
    ...
    async function handleLoginPage(request) {
        ...
        if (request.method === 'POST') {
            try {
                ...
                const token = await createAdminToken(adminPassword, request);
                const cookieExpiry = new Date(Date.now() + 3600000); // 1 小时
                const redirectTo = url.pathname.replace(/\/api\/login.*$/, '') || '/';
                ...
                return new Response(JSON.stringify({
                    ...
                    headers: {
                        'Content-Type': 'application/json',
                        'Set-Cookie': `cfnew_admin_token=${token}; path=/; expires=${cookieExpiry.toUTCString()}; Secure; HttpOnly; SameSite=Lax`
                    }
                });
            } catch (error) {
                return new Response(JSON.stringify({
    ...
    // 管理员登出 API：服务端清除 cookie（兼容 HttpOnly）
    async function handleLogoutAPI() {
        return new Response(JSON.stringify({ success: true }), {
            status: 200,
            headers: {
                'Content-Type': 'application/json',
                'Set-Cookie': 'cfnew_admin_token=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; Secure; HttpOnly; SameSite=Lax'
            }
        });
    }
